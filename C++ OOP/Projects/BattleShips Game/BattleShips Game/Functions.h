...
void HardBotHit(char**& area) {
    ...
    if (hit) {
        ...
        else { int randRotation = rand() % 4;
		if (((randRotation == 2 && ifRev == -1) || ifRev == 2) && c + 1 < column - 1) if (area[r][c + 1] == ship) { area[r][++c] = 'H'; hitBotC++; ifRev = 2; }
		else if (area[r][c + 1] == ' ') {
			area[r][c + 1] = 'M'; hit = false; start = true; ifRev = -1;} else if (area[r][c + 1] == 'M' || area[r][c + 1] == 'H') {hit = false;ifRev = -1;}
		else hit = true;
        else if (((randRotation == 3 && ifRev == -1) || ifRev == 3) && c - 1 > 0) if (area[r][c - 1] == ship) { area[r][--c] = 'H'; hitBotC++;ifRev = 3; } else if (area[r][c - 1] == ' ') {
            area[r][c - 1] = 'M'; hit = false; start = true; ifRev = -1;} else if (area[r][c - 1] == 'M' || area[r][c - 1] == 'H') {hit = false;ifRev = -1;}
		else hit = true;
        else if (((randRotation == 1 && ifRev == -1) || ifRev == 1) && r - 1 > 0) if (area[r - 1][c] == ship) { area[--r][c] = 'H'; hitBotC++; ifRev = 1;}else if (area[r - 1][c] == ' ') {
            area[r - 1][c] = 'M'; hit = false; start = true; ifRev = -1;} else if (area[r - 1][c] == 'M' || area[r - 1][c] == 'H') {hit = false;ifRev = -1;}
		else hit = true;
        else if (((randRotation == 0 && ifRev == -1) || ifRev == 0) && r + 1 < row - 1) if (area[r + 1][c] == ship) { area[++r][c] = 'H'; hitBotC++; ifRev = 0;} else if (area[r + 1][c] == ' ') {
            area[r + 1][c] = 'M'; hit = false; start = true; ifRev = -1;} else if (area[r + 1][c] == 'M' || area[r + 1][c] == 'H'){hit = false;ifRev = -1;}
		else hit = true;}
...
void normalBotHit(char**& area) {
	...
	if (hit) {
		...
		if (randRotation == 2 && c + 1 < column - 1) if (area[r][c + 1] == ship){ area[r][++c] = 'H'; hitBotC++;}else if (area[r][c + 1] == ' ') {
			area[r][c + 1] = 'M'; hit = false; start = true;}else hit = true;
		else if (randRotation == 3 && c - 1 > 0) if (area[r][c - 1] == ship){ area[r][--c] = 'H';  hitBotC++;}else if (area[r][c - 1] == ' ') {
			area[r][c - 1] = 'M'; hit = false; start = true;}else hit = true;
		else if (randRotation == 1 && r - 1 > 0) if (area[r - 1][c] == ship){ area[--r][c] = 'H'; hitBotC++;}else if (area[r - 1][c] == ' ') {
			area[r - 1][c] = 'M'; hit = false; start = true;}else hit = true;
		else if (randRotation == 0 && r + 1 < row - 1) if (area[r + 1][c] == ship){ area[++r][c] = 'H'; hitBotC++;}else if (area[r + 1][c] == ' ') {
			area[r + 1][c] = 'M'; hit = false; start = true;}else hit = true;}
	}else if (area[randR][randC] == ship) {
		area[randR][randC] = 'H';
		hitBotC++;
