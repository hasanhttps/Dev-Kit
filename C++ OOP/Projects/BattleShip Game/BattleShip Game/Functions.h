...
void normalBotHit(char**& area) {
	...
	if (hit) {
		...
		if (randRotation == 2 && c + 1 < column - 1) if (area[r][c + 1] == ship) area[r][c + 1] = 'H'; else if (area[randR][randC] == ' '){
			area[r + 1][c] = 'M'; hit = false; start = true;}
		else if (randRotation == 3 && c - 1 > 0) if (area[r][c - 1] == ship) area[r][c - 1] = 'H'; else if (area[randR][randC] == ' '){
			area[r + 1][c] = 'M'; hit = false; start = true;}
		else if (randRotation == 1 && r - 1 > 0) if (area[r - 1][c] == ship) area[r - 1][c] = 'H'; else if (area[randR][randC] == ' '){
			area[r + 1][c] = 'M'; hit = false; start = true;}
		else if (randRotation == 0 && r + 1 < row - 1) if (area[r + 1][c] == ship) area[r + 1][c] = 'H'; else if (area[randR][randC] == ' '){
			area[r + 1][c] = 'M'; hit = false; start = true;}
...
void guessCoord(char**& chooseArea, char**& botArea) {
	...
	while (status) {
		...
		case 80:
			...
			break;
		case '\r':
			if (botArea[curX][curY] == ship) {
				chooseArea[curX][curY] = 'H';
				hitC++;
			}else if (chooseArea[curX][curY] == ' ') {
	...
	for (int i = 0; i < row; i++) {
		...
		for (int j = 0; j < column; j++) {
			if (i == curX && j == curY && area[i][j] == ' ') {
				...
				cout << 'M' << ' ';
				color(defColor);
			}else if (area[i][j] == 'H' && botArea[i][j + 1] != ship && botArea[i][j - 1] != ship ||
...
void fillRand(char**& area) {
	int randR, randC;
	bool status = true, isreverese = false;
	...
	while (destroyer--) {
		...
		while (status) {
			...
			if (area[randR][randC] != ship && area[randR - 1][randC - 1] != ship && area[randR - 1][randC] != ship && area[randR - 1][randC + 1] != ship && area[randR - 1][randC + 2] != ship &&
				...
				area[randR + 1][randC + 1] != ship && area[randR + 1][randC + 2] != ship && randC < column - 2) status = false;
		}status = true;
		area[randR][randC] = ship;
		...
		while (status) {
			...
			if (area[randR][randC] != ship && area[randR - 1][randC - 1] != ship && area[randR - 1][randC] != ship && area[randR - 1][randC + 1] != ship && area[randR - 1][randC + 2] != ship &&
				...
				area[randR + 1][randC - 1] != ship && area[randR + 1][randC] != ship && area[randR + 1][randC + 1] != ship && area[randR + 1][randC + 2] != ship && area[randR + 1][randC + 3] != ship && randC < column - 3) status = false;
		}status = true;
		...
		while (status) {
			...
			if (area[randR][randC] != ship && area[randR - 1][randC - 1] != ship && area[randR - 1][randC] != ship && area[randR - 1][randC + 1] != ship && area[randR - 1][randC + 2] != ship &&
				...
				area[randR][randC + 4] != ship && area[randR + 1][randC - 1] != ship && area[randR + 1][randC] != ship && area[randR + 1][randC + 1] != ship && area[randR + 1][randC + 2] != ship && area[randR + 1][randC + 3] != ship && area[randR + 1][randC + 4] != ship && randC < column - 4) status = false;
		}status = true;
...
void fillManual(char**& area) {
	...
	while (true) {
		...
		cout << battleship + carrier + submarine + destroyer << endl;
		int arrow = _getch();
		switch (arrow) {
		case 72:
			if (choose != 0) choose--;
